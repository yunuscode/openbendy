...
struct SettingsView: View {
                ...
                HStack(spacing: 12) {
                    ForEach(BendStyle.allCases) { style in
                        Button { prefs.style = style } label: {
                            VStack(spacing: 7) {
                                PreviewEffect(params: .init(angle: 62, clearAngle: 135, perspective: style == .silk ? 0 : 0.3, blur: style == .frost ? 0.8 : 0.65, shadow: style == .shade ? 1 : 0.2, style: style))
    ...
    private var about: some View {
        VStack(alignment: .leading, spacing: 20) {
            ...
            Text("Version 1.0.2").font(.system(size: 11)).foregroundStyle(.secondary)
        }
    }
}
